...
int sigsend(int pid, int signum){
   ...
   //in case the PID wasn't found or the signal was already pending
   release(&ptable.lock);
   return -1;
}
...
void check_signals(struct trapframe *tf){
 ...
 // if (p->pending_signals == 0)
    ...
    /*here we make sure that's the highest waiting signal will be
        handled. if there are no signals in line we return*/
